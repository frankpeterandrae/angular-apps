...
export class ColorGridComponent implements AfterViewInit, OnInit, OnChanges {
	...
	private updateItemSize(): void {
		const firstCard: HTMLElement = this.viewPort?.elementRef?.nativeElement?.querySelector('.color-tile');
		if (firstCard) {
			const marginTop = parseInt(window.getComputedStyle(firstCard).marginTop, 10) || 0;
			const marginBottom = parseInt(window.getComputedStyle(firstCard).marginBottom, 10) || 0;
